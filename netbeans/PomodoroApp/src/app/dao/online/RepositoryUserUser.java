...
public class RepositoryUserUser implements app.dao.RepositoryUserUser {

    private final String API_URL = "https://parseapi.back4app.com/classes/UserUser";
    ...
    public RepositoryUserUser() {
    }

    @Override
    public void create(UserUser relation) throws Exception {
    ...
    @Override
    public void delete(UserUser relation) throws Exception {
        OkHttpClient client = new OkHttpClient();

        String where = "{\"idUserStart\":\"" + relation.getIdUserStart()
                + "\",\"idUserFinish\":\"" + relation.getIdUserFinish() + "\"}";
    ...
    public List<User> getFriendsById(String idUser) throws Exception {
        ...
        String where = "{\"idUserStart\":\"" + idUser + "\"}";
        ...
        try (Response response = client.newCall(request).execute()) {
            if (response.isSuccessful() && response.body() != null) {
                ...
                for (int i = 0; i < results.size(); i++) {
                    ...
                    User user = new User();
                    user.setObjectId(obj.get("idUserFinish").getAsString());
                    friends.add(user);
                }
            }
        } catch (IOException ex) {
